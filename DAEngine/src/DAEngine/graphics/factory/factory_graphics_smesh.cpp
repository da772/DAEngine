...
namespace da
{
	...
	da::CStaticMesh* CStaticMeshFactory::create(const std::string& path, bool process)
	{
		CHashString hash(path.c_str(), path.size());
#ifdef DA_GRAPHICS_BGFX
		return ms_factory.addInstance(hash, [&path, process]() { return new da::CBgfxStaticMesh(path, process); });
#endif
	}
