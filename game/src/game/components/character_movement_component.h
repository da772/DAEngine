...
public:
	CCharacterMovementComponent(const da::CGuid& guid, da::CEntity& parent);
...
private:
	da::ICharacterController* m_character = nullptr;
	da::CPhysicsShapeCapsule* m_shape = nullptr;
	glm::vec3 m_direction = glm::vec3(0.f);
	float m_rotateDir = 0.f;
	float m_setRotate = 0.f;
	float m_speed = 4.4f;
	float m_rotateSpeed = 128.f;
	da::FCharacterTuning m_tuning = { .15f };
