...
#define TEST_FUNC(x) {const uint64_t strt = utility::GetTimeUS(); if (x()) Logger::Log("[%s] [PASS] [%s] Runtime %.3f ms", utility::CurrentDateTime().c_str(), #x, (utility::GetTimeUS()-strt)/1000.f);}\
