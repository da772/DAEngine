...
#define TEST_ASSERT(x) if (!(x)) { CLogger::Log("[%s] [FAIL] [%s] Test Failed at %s : LINE %d : %s", utility::CurrentDateTime().c_str(), __FUNCTION__, __FILE__, __LINE__, #x); assert(x); return false; }
