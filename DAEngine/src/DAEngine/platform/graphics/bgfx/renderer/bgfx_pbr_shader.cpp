...
namespace da::platform::bgfx {
	...
	void CBgfxPbrShader::initialize()
	{
		...
		albedoLUTProgram = CBgfxGraphicsMaterial("shaders/cluster/cs_multiple_scattering_lut.sc");
		albedoLUTProgram.initialize();
	}
	...
	void CBgfxPbrShader::generateAlbedoLUT()
	{
		...
		::bgfx::dispatch(0, { albedoLUTProgram.getHandle() }, ALBEDO_LUT_SIZE / ALBEDO_LUT_THREADS, ALBEDO_LUT_SIZE / ALBEDO_LUT_THREADS, 1);
	}

	uint64_t CBgfxPbrShader::bindMaterial(const FBgfxMaterial& material)
	{
	...
	void CBgfxPbrShader::bindAlbedoLUT(bool compute /*= false*/)
	{
		if (compute)
